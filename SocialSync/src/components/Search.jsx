...
export default function Search() {
  ...
  const [searchName, setSearchName] = useState(''); // Add searchName state

  const handleNameSubmit = async (e) => {
    e.preventDefault();
    if (searchName) {
      setSearchDisplay(searchName);
      GetEventsByName(searchName); // Update to call GetEventsByName function
    }
  }
  ...
  const GetEventsByName = async (name) => {
    try {
      const response = await axios.get(`http://localhost:3001/events?name=${name}`); // Adjust the API endpoint URL
